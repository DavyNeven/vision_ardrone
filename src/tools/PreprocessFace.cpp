...
PreprocessFace::PreprocessFace(const char *shapePredictorFileName) {
    dlib::deserialize(shapePredictorFileName) >> shapePredictor;
    cout << "Loaded the shapePredictor from Dlib" << endl;
}

void PreprocessFace::detectBothEyes(const Mat &face, Point &leftEye, Point &rightEye) {
...
Mat PreprocessFace::getPreprocessedFace(Mat &faceImg, int desiredFaceWidth) {
